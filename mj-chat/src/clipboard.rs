...
fn read_wsl_clipboard_with_script(script: &str) -> Result<ClipboardContent> {
    ...
    if output.status != 0 {
        ...
        if detail.contains("ExternalException") {
            bail!(
                "Windows clipboard is busy or locked; try again"
            );
        }
...
mod tests {
    ...
    fn decodes_png_payload_larger_than_a_pipe_buffer() {
        ...
        let mut command =
            mj_core::targets::CommandSpec::new("cat", [fixture.to_string_lossy().into_owned()]);
        command = command.purpose("test large clipboard image helper");
        let process =
            mj_core::targets::CancellableProcessExecutor::with_timeout(Duration::from_secs(3));
...
"#,
        ...
        let output =
            mj_core::targets::CancellableProcessExecutor::with_timeout(WSL_CLIPBOARD_TIMEOUT)
                .execute(&command)
