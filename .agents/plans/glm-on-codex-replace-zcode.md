...
- [x] (2026-09-15 19:10Z) Milestone 7, stage one (commit 6f73fe58): `CodexProviderKind { Zai, DeepSeek, Other }` with `CodexProvider::kind()`; `zai_usage::serves_quota` routes through it; a Codex provider without a quota endpoint reports the usage-priced `API` reading instead of an error; `utility_llm.rs` resolves a private `UtilityFamily` per profile so a DeepSeek-on-Codex profile serves as a utility model with precedence 1, the `deepseek-*flash*` family match, and a backend built from the provider's base URL and `env_key`. Z.ai and other API-key providers stay excluded.
- [x] (2026-09-15 19:15Z) Milestone 7, documentation (commit 732047a7): every page, both READMEs, the hero component, the social card, and the Astro SEO text describe five harness kinds; DeepSeek appears only as a Codex custom provider that serves as a utility model and shows `API` for quota. `npx astro check` reports 0 errors.
- [x] (2026-09-15 19:50Z) Milestone 7, stage two (commit 5463a82e): `HarnessKind` has five variants. Deleted the DSH native codec (`mj-checkpoint/src/native/deepseek.rs`), the import reader (`mj-controller/src/import/deepseek.rs`), the bundled `@deepseek-ai/dsh` npm pin and its Containerfile layer, `PlanControlError::DeepseekUnsupported`, the `.credentials.yaml` validator, the `DSH_PERMISSION_MODE` enforcement, the `mj import deepseek` subcommand, and the TUI dropdown entry. Migration 11 and the tolerated `'deepseek'` CHECK value stay with a comment; the removed-harness database test now covers a `deepseek` row. `zstd` left `mj-checkpoint` and `serde_yaml` left `mj-core`. `cargo test` 3,378 passed, clippy clean.
- [x] (2026-09-15 20:05Z) Live validation of Milestone 7 in an isolated instance (commit cd007f98 adds the reusable test): with the user's real `deepseek-codex` profile, `mj doctor` reports it authenticated, `mj models --profile deepseek-codex` lists `deepseek-flash` and `deepseek-v4-pro` with efforts low/high, the viewer snapshot shows `"quota": {"summary": "API", "has_error": false}` for the profile, and `utility_llm_live_deepseek_codex` resolved the profile as a healthy DeepSeek-family candidate and completed a real compaction on `deepseek-flash`.
- [x] (2026-09-15 17:32Z) Deployment, part two: the live daemon now runs 2.9.0 (`mj daemon status` printed `Mjolnir daemon 1106557 (version 2.9.0) started 2026-09-15T17:32:39Z`), and `mj doctor` reports `glm` and `deepseek-codex` authenticated.
- [ ] Deployment, part three (after Milestone 7 stage two ships in a release): before upgrading the live daemon, remove `[profiles.deepseek]` (kind `deepseek`) from `~/.config/mjolnir/config.toml` and finish or close the five running DSH sessions (6eb3b5fd on `precision-3260`; 9cc543b3, 662647ea, a0e2daef, d051f3ad on `morannon-podman`, three of them with attached `mj wait` processes). A `deepseek` session row becomes an unsupported-harness row that the controller skips while its detached worker keeps running.
...
- Observation: The `deepseek-codex` profile shows `Unavailable` in the new-session dialog even though `mj doctor` reports it authenticated and the live daemon already runs 2.9.0. The label is the quota column, not an eligibility verdict: `mj-tui/src/lib.rs` `profile_choice` appends `ProfileQuota::compact()`, and `mj-client/src/quota.rs` renders every quota error as the word `unavailable`. The error comes from the custom-provider arm in `mj-controller/src/quota.rs`, which returns `Err("quota reporting is not available for model provider ...")` for any host that is not a Z.ai host. The DSH harness arm returned the usage-priced report (`extra: Some("API")`, no error) instead, which renders as `API` and which `classify_quota` in `mj-controller/src/utility_llm.rs` treats as healthy; an error report is classed Unknown with score 0, so a DeepSeek-on-Codex utility candidate would rank last without the fix.
  Evidence: `mj daemon status` printed `version 2.9.0`; `mj doctor` printed `Harness profile deepseek-codex: /home/jonathan/.codex-deepseek is present and authentication is available`; the profile picker builds from `config.enabled_profiles()` with no filter (`mj-tui/src/wizards.rs`); `quota_error_label` falls through to `"unavailable"`.

- Observation: anvil's `OpenAiClient::api_url` appends `/v1` only when the base URL does not already end with `/v1`, so the provider base URL `https://api.deepseek.com/v1` can be passed verbatim to `with_deepseek_reasoning_support`. The earlier plan note that API-key profiles need an anvil change applies to Z.ai (chat lives under `api/coding/paas/v4`), not to DeepSeek.
  Evidence: `crates/anvil-llm/src/llm_client.rs` line 1861 `if self.base_url.ends_with("/v1")`, with tests `api_url_appends_v1_when_missing` and `api_url_does_not_double_v1`.

- Observation: The live store holds 34 `deepseek` session rows, five of them `running` (6eb3b5fd on `precision-3260`; 9cc543b3, 662647ea, a0e2daef, d051f3ad on `morannon-podman`), and three `mj wait` processes are attached to the last three. The live config still has `[profiles.deepseek]` with `kind = "deepseek"` and `deepseek-codex = true` under `[subagents.eligible_profiles]`.
  Evidence: `sqlite3 -readonly ~/.local/share/mjolnir/mj.sqlite3 "select substr(session_id,1,8), state, last_profile, target_template_id from sessions where harness_kind='deepseek'"`; `ps` listing of `mj wait --session ...`.
...
- Decision: Remove the DSH harness (`HarnessKind::Deepseek`) and serve DeepSeek only through Codex.
  Rationale: The user found DSH does not support most of what Mjolnir wants from a harness UI, and DeepSeek serves the Responses API, so the Codex path covers it. Same treatment as ZCode: code, install, container layer, import, and docs go; `'deepseek'` stays tolerated in the database CHECK constraint.
  Date/Author: 2026-09-15, user and Claude.

- Decision: A custom-provider Codex profile serves as a utility model when its provider host is one anvil's chat-completions client can reach with the same base URL; today that is DeepSeek. Z.ai stays excluded until anvil can use a base URL verbatim.
  Rationale: Anvil appends `/v1` unless the base already ends in `/v1`. DeepSeek's Codex base `https://api.deepseek.com/v1` therefore yields the same chat endpoint anvil already uses for DSH. Z.ai's plan key only works for chat completions under `api/coding/paas/v4`, which anvil cannot form. Deriving a provider kind from the host in one place (`codex_provider.rs`) lets quota and utility selection share the knowledge instead of each matching on host strings.
  Date/Author: 2026-09-15, user and Claude.

- Decision: A Codex custom provider that publishes no quota endpoint reports the usage-priced `API` reading (no windows, `extra: "API"`, no error) instead of an error.
  Rationale: This is what the DSH harness arm did. An error renders as `unavailable` in every profile picker and makes the utility ranker score the profile as Unknown, both of which misdescribe a pay-as-you-go provider that is working.
  Date/Author: 2026-09-15, Claude.

- Decision: Only an API-key Codex profile whose provider kind is `DeepSeek` serves as a utility model; `Zai` and `Other` stay excluded.
  Rationale: DeepSeek's `/v1` base URL is the same chat-completions endpoint the shared OpenAI client speaks, so the client works unchanged. Z.ai's plan key serves chat only under `api/coding/paas/v4`, which still needs the anvil change recorded in `Outcomes & Retrospective`. `Other` providers are unknown and are not claimed to work.
  Date/Author: 2026-09-15, Claude.
...
Milestone 7 (2026-09-15): the DSH harness is gone from code, assets, the
container image, and the documentation, following the ZCode precedent. DeepSeek
is served by an ordinary Codex profile pointed at `https://api.deepseek.com/v1`.
Such a profile now shows `API` in every quota column instead of `unavailable`,
and serves as a utility model with the lowest precedence on its newest flash
model; this closes the DeepSeek half of the earlier "API-key profiles are
excluded from utility duty" limitation. Z.ai profiles are still excluded, for the
anvil reason recorded above. Not exercised live: a fresh `agent-dev` image
build, and the live upgrade itself, which waits on the deployment step above
(remove `[profiles.deepseek]`, close five running DSH sessions).
...
### Milestone 7: remove DSH and add the DeepSeek-on-Codex utility backend

Add a `CodexProviderKind` enum to `mj-core/src/codex_provider.rs` with variants `Zai`, `DeepSeek`, and `Other`, plus `CodexProvider::kind()` derived from the base URL host: `api.z.ai` and `open.bigmodel.cn` are `Zai`, `api.deepseek.com` is `DeepSeek`, anything else `Other`. Route the existing quota host check in `mj-controller/src/quota.rs` through it.

In `mj-controller/src/utility_llm.rs`, `backend_for_profile` for a Codex profile with `AuthScheme::ApiKey` now matches on the provider kind: `DeepSeek` builds `OpenAiClient::with_deepseek_reasoning_support(provider.base_url, key, HeaderMap::new())` with the key from `profile.environment[env_key]`; `Zai` and `Other` return `Ok(None)`. `utility_precedence` gives such a DeepSeek profile the value DSH had (1), and `family_matches` accepts ids that start with `deepseek-` and contain `flash`, carried over from the DSH arm. Both functions take the profile, not only the kind, where they need the provider. Keep the existing tests' intent by re-pointing the DeepSeek cases at a Codex profile whose home names the DeepSeek provider.

Then delete `HarnessKind::Deepseek` and every arm that matches it, following the same sweep as Milestone 3. Run `grep -rn "Deepseek\|deepseek\|\bdsh\b\|DSH_" --include=*.rs --include=*.toml --include=*.json --include=*.md --include='Containerfile*' .` (excluding `.mj/`, `target/`, and this plan) and expect remaining hits only where DeepSeek is a Codex provider (the new provider kind, the utility backend, its tests, and the docs' DeepSeek-on-Codex section), the historical migration text and the tolerated `'deepseek'` constraint value in `mj-controller/src/database/schema.rs`, and `.agents/plans/add-*.md` history. The pieces to remove include: `DSH_HOME` in `home_env`, `.dsh` in `default_home_leaf`, `.credentials.yaml` as the marker, `DSH_PERMISSION_MODE` enforcement, the `--profile acp` bridge args, the `dsh web` login command, the credential-freshness and expiry arms, the `deepseek_key` reader in `utility_llm.rs`, the managed DSH install and preflight, the `@deepseek-ai/dsh` layer in `containers/Containerfile.agent-dev` and the matching pin test, the `mj import deepseek` subcommand and its transcript reader, the TUI dropdown entry, the staging allowlist arm, skills mapping, checkpoint arms, the docs rows and the `mj import` table. `HarnessKind::ALL` becomes five variants. Add a comment beside the CHECK constraint that `'deepseek'` is retained only so rows from earlier releases load, exactly as for `'zcode'`; the existing unsupported-row test can gain a `deepseek` case.

Docs: `docs/src/content/docs/profiles.md` loses the DSH row and gains nothing new beyond the DeepSeek-on-Codex example already present; `cli-reference.md` loses `deepseek` from the import list; note in the profiles page that a DeepSeek-on-Codex profile can serve as a utility model while a Z.ai one cannot yet.

Deployment note for Milestone 6 part two: before the live daemon is upgraded past this milestone, the user's `[profiles.deepseek]` (kind `deepseek`) block must be removed from `~/.config/mjolnir/config.toml`, and any running DSH session (there is one on `precision-3260` at the time of writing) must finish or be closed first, because a `deepseek`-kind row becomes an unsupported-harness row that the controller skips while its detached worker keeps running.
...
11. After Milestone 7: `mj import deepseek` is no longer a subcommand; a config with `kind = "deepseek"` fails to load with "unknown harness kind"; a `deepseek-codex` profile appears in the utility-model candidate list (the `utility_llm` tests) with precedence 1 and matches `deepseek-flash`; a store with an old `deepseek` session row lists its other sessions.
