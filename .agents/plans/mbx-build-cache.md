...
- Resolution (commit): the session-worker now restores the container image's declared `ENV` onto the discovered login environment for container targets, via `login_environment::overlay_image_environment`, gated by `WorkerLaunchConfig.seed_image_environment` (true only for container kinds). It runs before the environment-clearing re-exec, so the harness, terminals, and the reviewer all see the restored variables; bare and localhost targets keep the clean-slate seed. Verified on localhost: the harness sysroot moved from `/home/hel/.rustup` to `/usr/local/rustup`, `PLAYWRIGHT_BROWSERS_PATH` is present, and a musl build finishes. The original observation follows.
