...
- [ ] M2 Add the `ResumeTab::Live` variant and `ResumeRowStatus::Running`, build Live rows from every workspace on the `DashboardState` side, give the Live tab its own substring search, and make Enter focus the selected session.
- [ ] M3 Replace the PROFILE and TARGET columns with a WORKSPACE column on the Live tab only.
- [ ] M4 Open the dialog on the Live tab, add the state filters, rename the command's user-facing words, update the documentation, and write the retrospective.
...
Open on Live: change `tab: ResumeTab::Hel` at line 875 to `ResumeTab::Live`. The dialog opens with its search box focused when the index is ready (`search_focus_pending`); with `search_enabled` now true on Live regardless of the index, confirm the box takes focus immediately on open.
