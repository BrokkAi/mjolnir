...
- [x] (2026-09-19 01:44Z) M5 Live opens with the list focused; the Sessions pane button reads **Open**; the modal title reads **Sessions**. Commit `fd1a4aa0`; 629 crate tests, workspace suite green across 31 crates, clippy clean. The search-focus flag turned out to have no live path — the only production construction of the dialog is `show_resume_dialog`, always on Live — so `search_focus_pending` and both `take_pending_*` methods were removed rather than guarded, and `apply_wiki_search` no longer touches focus. A sentence in `sessions.md` promising the box "opens by itself when the build finishes" described that removed mechanism and was corrected.
...
Closed 2026-09-19 after five milestones and five commits (`bc38c55b`, `6a245c54`, `7f4ad4cd`, `ccbf19bf`, `fd1a4aa0`). Against the Purpose: `prefix+g` now opens on a Live tab listing every running session in every workspace with a WORKSPACE column, the list has focus so `j`/`k`, Enter and the `a/b/w/i/d` letters work at once, `/` searches by name without waiting for the index, and Enter on a session in another workspace switches tabs and opens it through the same `focus_session_anywhere` the attention key uses. The three history lists sit one `→` away with their old columns. The command is called Sessions, the pane button Open, the configuration key still `resume`. Every behavior above has a test that failed before its milestone and passes after; the workspace suite is green.

Remaining limitations, all recorded in the docs rather than hidden: the preview pane shows a running session's conversation only when an index search matched it; and the state letter `b` on Live means "waiting or failed" exactly as in the Sessions pane, which the isolated test lab can reach only through failure, since its fake agent never asks a question.

What the milestones taught. A count that lives as a literal will be wrong in the place nobody grepped — the tab count was in five sites, one of them a mouse hitbox, and only `ResumeTab::COUNT` made the fifth visible. A "single search path" doc comment was load-bearing: reading it is what exposed that the Live tab would have emptied on the first keystroke. Extracting the cross-workspace jump before adding its second caller (M1) made M2 a small addition instead of a copy, and the one extra ownership fix it needed was found by the compiler, not by review. Changing which tab a dialog opens on invalidated 22 tests that assumed the old opening tab and one mechanism whose only trigger was that tab; when a flag's last trigger disappears, delete it — a guard would have left a focus-stealing path reachable from a late index answer. Finally, the plan's two review revisions (search path, build site, hit-count panic, index gate) each came from reading the code the plan cited rather than the plan's summary of it; a reviewer who trusts the summary ships the bug.
