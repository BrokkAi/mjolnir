...
- [x] (2026-09-07) Authenticated native checkpoint/restore passes with the saved Muse login: two provider turns through Mjolnir's client, random-token recall after restore, queued-work preservation, and credentials excluded from the archive (92.85 seconds).
- [x] (2026-09-07) Full agent image builds as localhost/mjolnir-muse-validation:4770957c (image 0a617b31e7d8). Default user hel runs both pinned binaries, adapter self-test and two native echo turns on one persisted session; LICENSE/NOTICE are present. Network disabled and no host credentials mounted.
- [x] (2026-09-07) Full cargo test, Clippy and formatting checks pass with the added ignored live regression; follow-up work is complete in the current-branch validation commit.
...
For authenticated native checkpoint coverage, set MJ_MUSE_ACP_TEST_BINARY to that adapter, MJ_MUSE_TEST_BINARY to the pinned native Muse executable, and MJ_MUSE_TEST_HOME to the logged-in account's config directory (normally /home/ryan/.config/muse). Run cargo test -p brokk-mj-core live_muse_checkpoint_restore -- --ignored --nocapture with elevated permissions. It makes two minimal provider turns through Mjolnir's actual ACP client, with shell and writes disabled. It copies authentication into a private temporary profile without printing it, checkpoints a random remembered token, verifies credentials are excluded, restores the archive and queued work into a new private profile, and asks for the token without repeating it. Both adapter/host processes close before the test profiles are removed. The first attempt exposed only a test assertion typo: Mjolnir's normalized success string is EndTurn, not the wire's end_turn.

Build the complete image using podman build --layers -t localhost/mjolnir-muse-validation:4770957c -f containers/Containerfile.agent-dev . with elevated permissions. This is a local-only validation tag, not a published image. Verify pinned binary startup, adapter self-test and offline native echo continuation under the image's default non-root user. No controller credentials are mounted into the container.
...
The upstream defect is fixed and published, and Mjolnir implementation and required validation are complete. The follow-up authenticated test now proves native context continuation after a real Mjolnir checkpoint/restore, with separate source and restored profile homes. It also verifies queued-work preservation and credential exclusion. Deterministic adapter tests cover permissions, questions, images and cancellation; selected child artifacts have fixture coverage, not paid-provider running-child continuation. The full Linux x86-64 container image builds and runs its pinned binaries and native persistence smoke as the default non-root user. Execution on remote/macOS targets is not claimed. Human-readable integration and limitations are documented in docs/src/content/docs/profiles.md and related reference pages.
...
Follow-up revision 2026-09-07: the user requested the remaining validation and saved a Muse login. Add reproducible authenticated checkpoint coverage and full-image validation rather than treating the previous smoke-test limitations as permanent.

Follow-up completion 2026-09-07: authenticated checkpoint/restore and full-image build/run are successful; this supersedes the earlier unclaimed live-provider and full-image coverage. The locally tagged validation image is retained for inspection. Mjolnir commits and image remain local; only the separately authorized adapter release was published.
