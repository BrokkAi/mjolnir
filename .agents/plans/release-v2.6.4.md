...
- [ ] Validate the clean release candidate locally and on exact-commit master CI.
- [ ] Verify registry publisher authorization before tagging.
- [ ] Push the annotated tag and verify GitHub, Rust, npm, and Homebrew publication.
...
Implementation and local environment repair are complete. Release preparation is in progress. npm publisher settings read returned HTTP 401 and the browser requires sign-in; no publisher configuration was changed. Same-day Rust evidence verifies all nine publishers.
