...
Milestone 1 instruments actor queueing, relay calls, durable projection, publication, terminal database reads/application/frame preparation, and browser projection. The reproducible probe measures endpoint responses and authoritative API availability, with optional real Playwright DOM and terminal frame timings. Use plain-text warm prompts and short/48 MiB histories on local and actual morannon-podman targets. Attachment preparation is not implicated by these reproductions and is not changed.

Milestone 2 fixes the measured presentation costs. Retain a session-local browser projector under the existing bounded background dispatcher; reuse decoded entries and avoid cloning hidden completed-tool bodies. Align terminal entry reuse by durable position as its canonical tail advances. Check source equality for same-millisecond updates and repairs. Preserve actor ordering, journal durability, projection generation guards, and lifecycle ownership. Add behavioral tests for collapse/history repair and preservation of computed diffstats across a sliding window.

Milestone 3 repeats the workload, records p50/p95, and validates the full existing correctness suite. Run `cargo test -- --quiet --test-threads=8` outside the sandbox, `cargo clippy --all-targets -- -D warnings`, `cargo fmt --check`, Python syntax/Ruff checks, and `npm test` in `tests/e2e/web`. Existing queue/reconnect/lease tests cover unchanged scheduling behavior. Commit only task-owned paths; existing unrelated plans, `1q`, and `mj.sqlite3` remain untouched. The user authorized merging and pushing; integrate upstream on master and validate the merged tree before pushing.
...
Upstream integration: committed implementation as `f92350f3`, then merged `origin/master` as `9cad8cf1` without conflicts. The merged tree passed Clippy and 83 deterministic browser tests (3 skipped), plus browser unit tests. The merged Rust run passed its unit suites but one startup-quit PTY test timed out under concurrent load. The exact failing binary passed in isolation (3.66 s), and all eight PTY tests passed serially (10.27 s), without source changes. Documentation tests were run separately because Cargo stopped at that transient integration failure. Logs use the `hel-latency-*-merged.log` names plus `hel-latency-pty-exact.log` and `hel-latency-pty-rerun.log` under `/mnt/optane`.
