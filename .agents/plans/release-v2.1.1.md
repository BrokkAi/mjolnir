...
Finally verify publisher access for every package, rerun `node scripts/release-version.mjs check v2.1.1` against clean master, create the annotated tag, and push it. Monitor the GitHub Release and registry workflows to completion; verify all three platform archives/checksums, eight crates, and four npm packages. Record outcome in this plan in a subsequent documentation commit without changing the release tag.
...
Release preparation underway; no tag created.
