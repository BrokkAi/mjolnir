...
The follow-up release request authorized publishing codex-acp 1.11.5 and updating mj. Version 1.11.5 is now published; the updated runtime, npm manifest/lockfile, and container pins enable native cancellation and the replay-generation fix. Claude currently advertises no child stop control; Kimi exposes lifecycle without child transcripts or stop. Permission requests remain on the owning session with native-child attribution; child plan requests cannot change the parent policy.
...
Follow-up release (2026-09-19): published `v1.11.5` from adapter commit `5792c82` using the normal workflow, https://github.com/BrokkAi/codex-acp/actions/runs/35482095362. Local npm ci/typecheck/592 tests/build/pack passed. Packaged guardian and yolo probes verified two main turns, preserved configured writable grants, automatic-review policy for guardian, and unrestricted execution without approval requests for yolo. The packaged JSON-RPC native cancellation endpoint also passed a live parent/sibling isolation check. GitHub's release tarball matches the locally tested artifact; npm's published integrity matches the mj lockfile, whose isolated npm ci passed. The exact managed pair (adapter 1.11.5 plus Codex 0.155.1) passed the native cancellation probe. Full Rust tests, all-target clippy, formatting, and the reliability fixture syntax check passed. The pin update is validated and ready for commit/push. This publishes the adapter only; no mj release or live-session restart was requested.
