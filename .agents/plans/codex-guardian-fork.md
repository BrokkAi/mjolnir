...
## Release execution update (2026-09-10)

The user completed npm login and browser publication approval. @brokkai/codex-acp 1.11.1 is public; clean npm ci using mj's production lock passed and both binaries reported their exact pinned versions. Trusted publisher readback confirms BrokkAi/codex-acp, publish.yml, environment release (ID 80753811-44fa-4ff4-9b78-7b93ca06812e). Adapter tag v1.11.1 and GitHub release workflow 34529362694 passed. Its GitHub tarball is byte-identical to the tested npm artifact.

Mj 2.6.2 manifests/lock/licenses were committed in 89dcccc8. Upstream capacity-retry work arrived before the release push. A conflict-free preview and its existing validation record supported a local merge in 1fdaf464; full serial tests including all six PTY tests, Clippy, optimized builds, generated licenses, and all nine package assemblies passed on that combined commit before pushing it. Automatic review initially rejected the combined merge-and-push command; splitting integration from validation and pushing only after the checks passed resolved the rejection. Image workflow 34530627897 is publishing amd64/arm64 from this commit.

Prior CI artifacts exposed baseline fixture failures: the Node preflight test lacked its Unix compile guard, and fake-harness fixtures lacked fake Node/npm executables required by launch preflight. Minimal test-only corrections restore deterministic fixtures. All six terminal tests, the preflight test, three-client reliability (zero leaks), and Clippy passed. No production mode behavior changed. Evidence is in target/release-v2.6.2-*.log. Final candidate CI and image verification must pass before tagging mj; archive, registry, and Homebrew publication remain outstanding.
