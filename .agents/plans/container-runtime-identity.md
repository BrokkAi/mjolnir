...
PR review findings (2026-09-26): Preserve unconstrained launches when `CODEX_PATH` is empty, retaining unknown provenance. Also run container npm preparation before initial worker startup and before upgrade idle admission: moving downloads into worker startup would otherwise hit the 60-second startup-step grace. Reuse the existing `prepare-harness` command and target copy helpers, stage upgrades under a unique private directory, and preserve the old executable/config/control socket on preparation failure. A failed exec connection may leave a live process, so retain its staging files on failure instead of deleting them. New behavior regressions cover the five container transports and cache readiness before worker preparation.
