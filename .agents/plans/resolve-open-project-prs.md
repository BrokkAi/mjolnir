...
- [ ] Commit the integrated project picker on master.
- [ ] Prepare #1148 and validate combined changes; publish after its final platform check succeeds, close superseded #1145 and clear coordination labels.
...
Keep single-repository selection immediate; only explicitly enabling multi-repository selection should collect sources before opening the project. The first source is primary and removal updates that order. Use the existing grouped `/api/bundles` endpoint, which already validates and persists the exact selection atomically.

## Outcomes & Retrospective

The project integration is implemented and validated. The full workspace run passed all unit and integration suites except a PTY assertion compiled before the master-compatible title adjustment; the corrected complete PTY target passes all 10 tests, and documentation tests pass separately. All-target Clippy, formatting, diff checks, Python fixture syntax, 51 Node tests and 110 deterministic browser tests pass. Terminal PR CI and final combined validation remain. Success means applicable changes are merged, superseded changes are closed with the unique browser capability preserved, and no open PR remains from the initial set.
