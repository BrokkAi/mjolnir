...
Issue #996 follow-up (2026-09-14 UTC). The issue reported that profile `claude2` advertised `opus` and `opus[1m]` in local discovery but only `opus` in a live container session, while `claude3` advertised only `opus[1m]` everywhere. The commit that resolved it (e964c798, "Resolve post-989 runtime tickets") recorded this as profiles that "genuinely vary by authenticated profile" and rejected a move that would drop the pin. That resolution is sound, but the recorded evidence was stale and the mechanism is the one above, not the account. `claude2` holds a setup token at `profiles/claude2/claude-oauth-token`; `claude3` has none and runs on copied login credentials. The issue was filed against v2.6.4, which predates this plan's change, so local discovery for `claude2` still used login credentials (both spellings) while the container used the token (`opus` only). After this plan's change, the discovery cache in `mj.sqlite3` shows `claude2` with the token offering `opus` only (2026-09-14 06:29Z), matching the container, and `claude3` with login credentials offering `opus[1m]` only. The accounts also differ under login credentials (`claude2` login offered both spellings at 03:53Z), so the move rejection remains correct; the move validation probe now runs with the same credential type the destination container receives. Two points remain unverified: the probe is a fresh session while a move resumes, and the earlier evidence shows those menus can differ under a setup token; and the issue's loss of the `xhigh` effort pin on v2.6.4 was never explained.
