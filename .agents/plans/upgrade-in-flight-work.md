...
Revision note (2026-09-22): Narrow the daemon handoff gate to daemon-owned work. The session relay actor and the turn-review host no longer hold upgrade admission, and the legacy pre-33 idle check now blocks only on `lifecycles` and on records in `Provisioning`, `Checkpointing`, `Closing`, or `Destroying`. Reason: the daemon is the control plane and workers are the data plane, so an unreachable worker or an ordinary busy session held daemon replacement open indefinitely, with no deadline. Worker replacement is unchanged and still requires atomic idle admission. A new `DaemonAction::UpgradeBlockers` and `DaemonReply::UpgradeBlockers(Vec<String>)` let the CLI's wait notice name the daemon-owned work it is waiting for. `DaemonStatus` could not carry the labels instead: every daemon protocol struct is `#[serde(deny_unknown_fields)]`, and already-shipped clients are compiled that way, so adding a field to a frozen management reply would make an older `mj` fail to parse a new daemon's `Status`. A new action is safe in the other direction, because an older daemon simply fails the unknown frame and the CLI treats that as "unknown" and prints the unnamed notice.
