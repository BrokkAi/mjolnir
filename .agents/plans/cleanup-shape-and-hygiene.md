...
- [x] (2026-09-16) Stage 3, part 2: sha2 0.11 with one hex helper, dead `MJ_DISCOVER_LOGIN_PATH` constant removed, five more `HarnessKind` fact methods (`mcp_config_file`, `supports_delegation_tools`, `native_session_dirs`, managed entrypoint, `marks_own_turn_end`), `local_sockets` cwd race serialized. Also hmac 0.13 (forced by digest 0.11) and `mj_core::hex::lower_hex` replacing four hand-rolled encoders. Merged; full suite 3,410 passed, 0 failed.
- [x] (2026-09-16) Stage 3, part 3: measurements re-taken, retrospective written.
- [ ] Open, not part of this plan: the three `mj_core::login_environment` tests fail intermittently under build load and pass alone (cause unknown; the status assertion now prints its error). `recovery_backend_locator`'s three drifts need an issue. `oauth2` pins sha2 0.10, rand 0.8 and thiserror 1, so those duplicates wait on upstream.
...
(2026-09-16) The plan is complete on `master`, 37 commits from 7fd533eb, none pushed. Every user-facing feature behaves as before: the full suite passes on the final tree (3,410 passed, 0 failed, clippy clean with `-D warnings`, rustfmt clean). Net line count outside `.agents` and `Cargo.lock` is +970 across 362 files, almost all module headers, imports and re-exports from the splits; the work moved code rather than removing it, which is what the shape goal asked for.

Before and after, measured the same way on 7fd533eb and on the final tree: production files over 3,000 lines went from 31 to 0, and none is over 2,000 (the largest is `controller/resume.rs` at 1,989). Inline test modules went from 184 to 125 and dedicated test files from 31 to 78. `impl HarnessKind` grew from 15 to 43 methods; `HarnessKind::Claude` references in production code outside mj-core fell from 206 in 44 files to 59 in 27 files, and every remaining match does something different per harness. The two stored-to-plan target conversions in mj-controller became one `TryFrom` in mj-core. `cargo machete` reports nothing (8 hits before). `[workspace.dependencies]` went from 13 to 42 entries with `Cargo.lock` unchanged by that step. The reqwest duplicate is gone; sha2 and its digest family stay duplicated because `oauth2` still pins sha2 0.10, and rand, thiserror, strum and itertools are transitive as predicted. Twelve self-spawning test files share one helper per crate (one raw `current_exe` spawn remains, inside the helper). Four flaky tests were fixed at the source: two ETXTBSY cases (issue #1036 and the npm-upgrade test), the long-root socket daemon-exit race, and the `local_sockets` working-directory race; the relay-proxy stderr tail lost its all-or-nothing cliff.

What the plan got wrong: the "160 hand-built constructions" figure counted match patterns; six of the seven environment-variable removals were test-only reads; "temp name and rename" does not cure ETXTBSY; the sha2 upgrade could not remove any duplicate; and `From<&state::TargetLocator>` was not constructible because the stored form omits what configuration holds. Each was caught by an agent that verified before acting, which the briefs required after the first surprise.

Lessons: measure code regions, not files, when a claim depends on `#[cfg(test)]`; a module split preserves reach only with `pub(super)`/`pub(in ...)`, not `pub(crate)`; agent worktrees must have their base checked and reset before any work, because five of six branched from another worktree's line; parallel agents need per-agent scratch directories; and `cargo test --no-fail-fast` is the only way a full run reports every crate.
