...
## Quota-aware continuation v2 (prepared locally)
...
The proxy retains `/v1/continuation-verdict` with its original two-question contract. The v2 route uses the existing continuation rate limiter and transport bounds. `npm test` and `npm run check` validate both versions. Deploy this route before distributing the updated client to hosted users; this change prepares the route but does not publish it. Direct TypeSafe callers send the new shared questions without requiring the hosted route.
