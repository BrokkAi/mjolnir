...
The public service is `https://mj-jev-proxy.eng-admin-a63.workers.dev`, deployed as `mj-jev-proxy` in the Brokk Cloudflare account. Its POST routes include `/v1/turn-verdict`, `/v2/turn-verdict`, `/v3/turn-verdict`, `/v1/help-search`, `/v1/continuation-verdict`, and `/v2/continuation-verdict`. All six routes are deployed. Source, pinned development dependencies, and Wrangler configuration live in `services/jev-proxy/` in the OSS repository. Deployment is independent of mj releases. GitHub CI validates changes but does not deploy them.
