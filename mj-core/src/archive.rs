...
fn expected_schema_version(payloads: &[PayloadDescriptor]) -> u32 {
    if payloads.iter().any(|payload| matches!(&payload.role, PayloadRole::NativeArtifact { relative_path } if relative_path.starts_with(crate::attachment::ARCHIVE_ATTACHMENT_DIR))) {
...
pub fn read_session_file(root: &Path, relative: &Path) -> Result<Vec<u8>, SessionExportError> {
    ...
    crate::config::validate_relative_destination(relative)
