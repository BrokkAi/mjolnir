...
Mjolnir is Codex-first, not Codex-only. You can add Claude, Kimi, OpenCode, or a
...
- **Integrated adversarial review:** challenge workspace changes with a
  ...
  turn completes.
- **Optional agent routes:** add Claude, Kimi, OpenCode, or a custom ACP server
...
Other agents are optional. Mjolnir can also use existing Claude, Kimi, or
OpenCode credentials and install supported binary ACP agents. Read
