...
pub use crate::hel_local_git::resolve_local_repository;
...
pub fn resolve_repository(
    ...
    match (&repository.github, &repository.local) {
        ...
        (None, Some(path)) => {
            #[cfg(feature = "controller")]
            {
                crate::hel_local_git::resolve_local_repository(path, executor)
            ...
            {
                let _ = (path, executor);
                bail!("local repository resolution is only available in the controller")
            }
        }
        (None, None) => bail!(
