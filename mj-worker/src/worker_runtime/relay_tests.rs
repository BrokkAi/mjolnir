...
use super::*;
use crate::acp::{CommandRequest, RuntimeEvent};
use crate::relay::{
...
fn skills_state_of(payload: RelayResponsePayload) -> mj_core::skills::SkillsSyncState {
    ...
    mj_core::skills::SkillsSyncState {
        present,
        fingerprint,
    }
}
...
fn github_cli_wrapper_reads_each_live_token_and_clears_stale_environment() {
    ...
    let invoke = |expected_token: Option<&str>| {
        match expected_token {
            Some(token) => {
                mj_core::credentials::write_github_token(&token_path, token.as_bytes()).unwrap();
            }
            None => mj_core::credentials::remove_github_token(&token_path).unwrap(),
        }
...
"#,
    ...
    direct
        ...
        .env_clear()
        .envs(&environment);
    let output = mj_core::subprocess::run_with_input(&mut direct, &[]).unwrap();
    assert!(
        output.status.success(),
        "{}",
        String::from_utf8_lossy(&output.stderr)
...
async fn credential_exchange_stays_on_the_connection_and_out_of_relay_state() {
    ...
    assert_eq!(
        fingerprint,
        mj_core::credentials::credential_fingerprint(&bytes)
    );
...
async fn incompatible_protocol_cannot_read_or_mutate_credentials() {
    ...
    mj_core::credentials::write_credential_file(endpoint.harness, &endpoint.marker, &original)
        .unwrap();
...
fn absent_reads_and_invalid_installs_are_refused() {
    ...
    let oversized = vec![b'a'; mj_core::credentials::MAX_CREDENTIAL_BYTES + 1];
    let error =
...
/// Elicitation answers share the ACP channel with durable commands. They must
/// not steal capacity between dispatch reserving slots and claiming work.
#[tokio::test]
async fn out_of_band_sends_cannot_steal_reserved_dispatch_capacity() {
    ...
    let mut user_shells = crate::user_shell::UserShellRegistry::new(
        temp.path().to_path_buf(),
        BTreeMap::new(),
        shell_events,
    );

    // Lock before starting dispatch itself. The coordinator also takes this
    // lock before dispatch, so racing its event loop can stall the test before
    // any permits are reserved.
    ...
    let holder = tokio::task::spawn_blocking(move || {
        ...
        // Dropping the sender on a failed assertion also releases the lock.
        let _ = release_rx.blocking_recv();
    });
    claiming_rx.await.unwrap();
    let dispatch = tokio::task::spawn_blocking(move || {
        unix::dispatch_pending(
            &relay,
            &command_tx,
            &mut BTreeMap::new(),
            true,
    ...
    wait_until(
        || out_of_band.capacity() == 0,
        "dispatch never reserved transport capacity before claiming durable work",
    )
    .await;
...
#[tokio::test]
async fn coordinator_drains_events_while_the_acp_command_channel_is_full() {
    let temp = tempfile::tempdir().unwrap();
    let mut durable = DurableRelay::open(temp.path(), SESSION_ID, "1.0.0").unwrap();
    submit(&mut durable, "prompt-batched", prompt("queued turn"));
    submit(&mut durable, "cancel-batched", RelayCommand::Cancel);
    let relay = Arc::new(Mutex::new(durable));
    let (event_tx, event_rx) = runtime_event_channel();
    let (wake_tx, wake_rx) = mpsc::channel(1);
    let (command_tx, mut command_rx) = mpsc::channel(2);
    ...
    event_tx
        .send(RuntimeEvent::Warning {
            message: "out-of-band channel full".into(),
        })
        .unwrap();
    wait_until(
        || recorded_warning(&relay, "out-of-band channel full"),
        "out-of-band commands stopped the coordinator from draining ACP events",
    )
    .await;

    // Once ACP makes room, the same accepted prompt and cancel still dispatch.
    for _ in 0..2 {
        assert!(matches!(
            next_command(&mut command_rx).await,
            CommandRequest::ResolveElicitation { .. }
    ...
    wait_until(
        || command_rx.len() == 2,
        "the coordinator never dispatched the queued commands",
    )
    .await;
    event_tx
        .send(RuntimeEvent::Warning {
            message: "durable channel full".into(),
        })
        .unwrap();
    wait_until(
        || recorded_warning(&relay, "durable channel full"),
        "durable commands stopped the coordinator from draining ACP events",
    )
    .await;
    assert_prompt(
        next_command(&mut command_rx).await,
...
fn terminal_lifecycle_journals_a_fallback_tool_and_tail_capped_output() {
    ...
    assert!(
        serde_json::to_vec(event).unwrap().len() <= mj_core::relay::RELAY_EVENT_BYTE_BUDGET,
...
async fn a_removed_worker_root_reports_a_fatal_failure_to_the_daemon() {
    ...
    encoded.push(b'\n');
    writer.write_all(&encoded).await.unwrap();
    let response: mj_core::relay::RelayResponseEnvelope =
        serde_json::from_str(&lines.next_line().await.unwrap().unwrap()).unwrap();
    assert!(
        matches!(
            response.body,
...
async fn client_disconnect_releases_checkpoint_and_runs_queued_prompt() {
    ...
    let response: mj_core::relay::RelayResponseEnvelope = serde_json::from_str(&response).unwrap();
...
async fn closed_relay_stays_attachable_after_the_acp_runtime_stops() {
    ...
    assert!(matches!(
        claimed.as_slice(),
        [mj_core::relay::ClaimedRelayCommand {
            command_id,
            command: RelayCommand::BeginCheckpoint { .. },
            ..
    ...
    durable
        .record_command_completed("close-command", mj_core::relay::RelayCommandOutcome::Closed)
        .unwrap();
    durable
...
async fn oversized_response_is_rejected_before_writing() {
    ...
    let response = RelayResponseEnvelope {
        ...
        body: RelayResponseBody::Error {
            error: RelayProtocolError {
                code: RelayErrorCode::Internal,
                message: "x".repeat(mj_core::relay::MAX_FRAME_BYTES),
                retryable: false,
                detail: None,
            },
        },
...
/// `cargo test --lib
/// worker_runtime::relay_tests::catch_up_recording_throughput
/// -- --ignored --nocapture`.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
...
async fn large_photo_transfer_is_verified_without_journaling_image_bytes() {
    use base64::Engine as _;
    use mj_core::attachment::{AttachmentRef, MAX_IMAGE_BYTES};
    let temp = tempfile::tempdir().unwrap();
    let relay_root = temp.path().join("relay");
    let relay = Arc::new(Mutex::new(
        DurableRelay::open(&relay_root, SESSION_ID, "1.0.0").unwrap(),
...
async fn checkpoint_only_daemon_preserves_work_and_seals_without_a_harness() {
    ...
    config.harness_runtime = mj_core::worker_launch::HarnessRuntimePolicy::Managed;
