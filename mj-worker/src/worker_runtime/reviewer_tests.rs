...
async fn applying_a_model_refreshes_the_advertised_efforts() {
    ...
    let efforts = crate::acp::session_config_choices(options, "effort")
        .into_iter()
...
async fn a_reviewer_form_is_answered_on_the_connection() {
    ...
    let body = fixture
        .request(ReviewerRequest::RespondElicitation {
            ...
            response: mj_core::elicitation::ElicitationResponse::Decline,
        })
        .await;
    assert!(
        matches!(
...
fn the_plain_relay_refuses_reviewer_requests() {
    let temp = tempfile::tempdir().unwrap();
    let mut relay = crate::relay::DurableRelay::open(temp.path(), SESSION_ID, "1.0.0").unwrap();
