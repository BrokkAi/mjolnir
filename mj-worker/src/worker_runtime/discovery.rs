...
use crate::acp::{self, AcceptedSessionConfig, CommandRequest, LaunchSpec, RuntimeEvent};
use anyhow::{Context, Result, bail};
use mj_core::config::ExecutionPolicy;
use mj_core::worker_launch::{HarnessRuntimePolicy, ProfileConfig, ProfileProbeSpec};
use tokio::sync::mpsc;
...
async fn probe_with_close_timeout(
    ...
    let mut task = tokio::spawn(acp::run_with_shutdown(
        launch,
        requests,
        events,
        shutdown.clone(),
        ...
        loop {
                ...
                .context("discovery harness stopped before reporting choices")?
            {
                RuntimeEvent::SessionConfigured { config_options } if initial_models.is_none() => {
                    let models = acp::session_config_choices(&config_options, "model");
...
fn discovered(
    ...
    models: Vec<acp::SessionConfigChoice>,
) -> ProfileConfig {
    let facts =
        acp::AcpSessionFacts::from_operational(harness, &Default::default(), &options, None);
    ProfileConfig {
        model: facts.current_model().map(str::to_owned),
        models,
        efforts: acp::session_config_choices(&options, "effort"),
...
mod tests {
    ...
    fn discovery_cleanup_errors_do_not_replace_results() {
        let config = discovered(mj_core::config::HarnessKind::Kimi, vec![], vec![]);
