...
impl Default for ViewerControl {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ViewerControl {
    ...
    pub fn access(&self) -> WebViewerAccess {
        self.access.borrow().clone()
    }

    pub fn publish(&self, access: WebViewerAccess) {
        self.access.send_replace(access);
    }

    pub fn recover(&self, action: WebViewerRecovery) -> Result<()> {
    ...
    pub fn conflict_address(&self) -> Result<SocketAddr> {
        match self.access() {
...
pub fn inspect_listener(address: SocketAddr) -> Result<Vec<WebListenerProcess>> {
...
#[cfg(not(target_os = "linux"))]
fn listener_pids(address: SocketAddr) -> Result<Vec<u32>> {
    use crate::targets::{CancellableProcessExecutor, CommandExecutor, CommandSpec};
