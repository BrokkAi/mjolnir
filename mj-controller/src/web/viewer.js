...
function renderProjectChooser(body) {
  ...
  if (home) {
    ...
    if (picker.mode === 'url') {
      ...
      const use = projectButton(newDraft.creatingBundle ? 'Opening project…' : 'Use repository', () => createNewProject(), 'new-project-use');
      body.append(use);
    } else {
    ...
    for (const entry of picker.entries) {
      const button = projectButton('', () => {
        if (entry.kind === 'directory') {
          ...
          createNewProject(entry.source);
        }
      });
      button.classList.add('project-result');
      button.append(el('span', '', entry.name), el('small', 'dim', entry.description || (entry.kind === 'directory' ? 'Open folder' : entry.source)));
      button.append(el('small', 'project-result-action', entry.kind === 'directory' ? 'Browse →' : 'Use repository →'));
...
async function createNewProject(selectedSource) {
  const draft = newDraft;
  ...
  renderNewForm();
  try {
    const result = await request('/api/bundles', { method: 'POST', signal: controller.signal, body: JSON.stringify({ sources: [source] }) });
    if (!current()) return;
