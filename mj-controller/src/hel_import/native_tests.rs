...
use hel::hel_archive::{PayloadRole, verify_archive_streaming};
use hel::hel_checkpoint::{CheckpointRestoreSpec, restore_checkpoint};
use hel::hel_config::{HarnessKind, HelConfig, ProjectBundle, ProjectRepository, TargetTemplate};
...
fn fixture(root: &Path, kind: HarnessKind) -> NativeFixture {
    ...
    let (home, source_path, native_session_id) = match kind {
        ...
        HarnessKind::Deepseek => {
            ...
            let project = hel::hel_native::deepseek::project_key(&cwd).unwrap();
            let encoded_id = hel::hel_native::deepseek::encode_segment(&id).unwrap();
            let path = home
                .join("sessions")
                .join(project)
                .join(encoded_id)
