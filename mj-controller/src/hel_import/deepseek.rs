...
pub(super) fn read_transcript(path: &Path) -> Result<ClaudeTranscript> {
        ...
        hel::hel_native::deepseek::is_session_log(path),
        "unsupported DeepSeek session artifact path {}",
        path.display()
    );
    if let Some(generation) = hel::hel_native::deepseek::session_generation(path)
        && generation != "0"
    {
        bail!(
    ...
    let log = hel::hel_native::deepseek::read(path, &data)?;
...
fn find_log(directory: &Path) -> Result<Option<PathBuf>> {
    ...
    for entry in fs::read_dir(directory)? {
        ...
        if !hel::hel_native::deepseek::is_session_log(&path) {
            continue;
        }
        let generation = hel::hel_native::deepseek::session_generation(&path)
...
fn validate_path_identity(path: &Path, header: &Value) -> Result<()> {
    ...
    let encoded_id = session_dir
        ...
        hel::hel_native::deepseek::decode_segment(encoded_id).as_deref() == Some(id.as_str()),
    ...
    let expected_project = hel::hel_native::deepseek::project_key(&cwd)?;
    ensure!(
