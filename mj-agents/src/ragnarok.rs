...
use mj_core::headless::choose_allow_option;
use mj_core::labels::stop_reason_label;
use mj_core::roster;
use mj_core::worktree;
...
pub const DISMISS_TIMEOUT: Duration = Duration::from_secs(3);
...
#[derive(Debug)]
pub struct SessionStartTimeout;
...
pub struct PromptToolLifecycle {
...
async fn battle(
    ...
    let cap = mj_core::config::Config::load(&mj_core::config::default_config_path())
        .map(|config| config.ragnarok.max_competitors)
        .unwrap_or_else(|_| mj_core::config::RagnarokConfig::default().max_competitors);
...
fn catalog_provider(model: &str) -> String {
    let provider = mj_core::deepswe::model_provider(model);
...
pub fn select_judge_only_reviewer(
    pool: &[Candidate],
    current_roster: &[Candidate],
    survivor: FighterId,
) -> Option<Candidate> {
...
pub enum TurnEvent {
    ...
    Permission {
        prompt: Box<mj_core::event::PermissionPrompt>,
...
impl AgentHandle {
    ...
    #[allow(dead_code)]
    pub async fn run_advertised_command(
        &mut self,
        name: &str,
    ...
    pub async fn connect_with_saved_session_config(
        launch: &Launch,
        cwd: &Path,
        additional_directories: &[PathBuf],
        abort: watch::Receiver<bool>,
    ...
    pub async fn connect_with_role_config(
        launch: &Launch,
        cwd: &Path,
        additional_directories: &[PathBuf],
        abort: watch::Receiver<bool>,
    ...
    pub async fn connect_with_role_config_and_mcp(
        launch: &Launch,
        cwd: &Path,
        additional_directories: &[PathBuf],
        abort: watch::Receiver<bool>,
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn connect_with_role_config_and_mcp_resuming(
        launch: &Launch,
        cwd: &Path,
        additional_directories: &[PathBuf],
        abort: watch::Receiver<bool>,
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn connect_with_role_config_and_mcp_resuming_with_startup_timeout(
        launch: &Launch,
        cwd: &Path,
        additional_directories: &[PathBuf],
        abort: watch::Receiver<bool>,
    ...
    fn answer_permission(&self, prompt: mj_core::event::PermissionPrompt) {
    ...
    /// resolution.
    pub async fn arm_model(&mut self, model_value: &str) -> Result<()> {
    ...
    fn model_is_current(&self, model_value: &str) -> bool {
        self.config_options.iter().any(|option| {
            mj_core::session_state::is_model_config_option(option)
                && mj_core::session_state::config_option_current_value_id(option)
    ...
    fn find_model_choice(
        ...
        for (option, target) in self.config_options.iter().zip(&self.config_targets) {
            if !mj_core::session_state::is_model_config_option(option) {
                continue;
            }
            let Some(choices) = mj_core::session_state::config_option_choices(option) else {
                continue;
            };
    ...
    pub async fn prompt_with_images(
        &mut self,
        text: String,
        images: Vec<mj_core::event::PromptImage>,
        budget: Duration,
...
pub fn permission_decision_for_access(
    access_mode: acp::RuntimeAccessMode,
    prompt: &mj_core::event::PermissionPrompt,
) -> PermissionDecision {
...
fn tool_kind_word(kind: Option<ToolKind>) -> &'static str {
    match kind {
        Some(k) => mj_core::labels::tool_kind_label(k),
        None => "tool",
    }
}
...
mod tests {
    ...
    async fn permissions_prefer_allow_then_reject_never_cancel_when_avoidable() {
        ...
        rig.handle
            .answer_permission(mj_core::event::PermissionPrompt {
    ...
    async fn forward_turn_event_respects_read_only_permission_policy() {
        ...
        fields.kind = Some(ToolKind::Edit);
        let (ptx, prx) = tokio::sync::oneshot::channel();
        let prompt = mj_core::event::PermissionPrompt {
            tool_call: ToolCallUpdate::new("edit", fields),
            options: vec![
                PermissionOption::new("deny", "Deny", PermissionOptionKind::RejectOnce),
                PermissionOption::new("allow", "Allow", PermissionOptionKind::AllowOnce),
