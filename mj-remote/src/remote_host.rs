...
use crate::remote::{self, RemoteSessionTracker, ServerSessionLaunchState};
use mj_agents::subagent;
use mj_core::acp::{self, AcpRuntimeConfig};
use mj_core::config::{self, SelectedAgent};
use mj_core::event::{UiCommand, UiEvent};
use mj_core::roster;
...
#[derive(Debug)]
pub struct RootServerSessionManager {
...
pub fn config_file_hash(config_path: &Path) -> Option<u64> {
...
fn start_server_agent_session(
    ...
    let (subagent_roles, subagent_codex_home) = match roster.as_ref() {
        Some(resolved) => {
            match mj_agents::live::isolated_subagent_roles(
    ...
    let quota_gate = mj_agents::quota::Gate::new(cwd.clone(), runtime_event_tx.clone());
    let subagent_pool = (!subagent_roles.is_empty()).then(|| {
        mj_agents::quota::RolePool::new(
            subagent_roles,
            quota_gate.clone(),
    ...
    let live_subagent_options = mj_agents::live::LiveSubagentOptions {
        agent_stderr: None,
    ...
    let live_subagent_service = match subagent_pool.clone() {
        Some(pool) => {
            subagent::LiveRuntimeService::new(mj_agents::live::configured_subagent_service(
                pool,
            ...
            review_fanout: match (
                ...
                (Some(workers), Some(supervisor)) => {
                    mj_core::orchestrator::ReviewFanout::available(
                        mj_agents::discrete_review::live_spawner(
                            mj_agents::discrete_review::FanoutConfig {
                                workers,
                                supervisor,
                ...
                (workers, supervisor) => {
                    let error = roster.as_ref().map_or_else(
                        ...
                        |resolved| {
                            mj_agents::live::review_fanout_error(
                                workers.is_some(),
                                supervisor.is_some(),
    ...
    let task = tokio::spawn(async move {
        ...
        let command_proxy = {
            ...
            tokio::spawn(async move {
                let mut side_runtime: Option<mj_core::side::Runtime> = None;
                ...
                while let Some(command) = server_cmd_rx.recv().await {
                    if let UiCommand::StartSide { initial_prompt } = command {
                        ...
                        let launch = mj_core::side::Launch {
                            agent: &side_agent,
                            cwd: side_cwd.clone(),
                        ...
                        let side = match mj_core::side::start(
                            launch,
                            &runtime_cmd_tx,
                            side_event_tx.clone(),
                        )
                    ...
                    if matches!(command, UiCommand::ExitSide) {
                        ...
                        if let Some(side) = side_runtime.take()
                            && let Some(message) =
                                mj_core::side::discard(side, &side_agent, None).await
                        {
                    ...
                    if !force_main && side_runtime.is_some() {
                        if matches!(command, UiCommand::Shutdown) {
                            if let Some(side) = side_runtime.take() {
                                tracker.finish_side_exit();
                                let _ = mj_core::side::discard(side, &side_agent, None).await;
                            }
                        } else {
                    ...
                    if matches!(command, UiCommand::ReloadAuxiliaryAgents) {
                        ...
                        if !mj_agents::live::primary_route_matches(
                            command_primary,
                            &updated_roster.primary,
                        ) {
                        ...
                        let (roles, codex_home) = match mj_agents::live::isolated_subagent_roles(
                        ...
                        if let Some(pool) = pool.as_ref() {
                            command_live_subagent_service
                                .replace(mj_agents::live::configured_subagent_service(
                                    pool.clone(),
                ...
                if let Some(side) = side_runtime.take() {
                    tracker.finish_side_exit();
                    if let Some(message) = mj_core::side::discard(side, &side_agent, None).await {
                        let _ = side_event_tx.send(UiEvent::Warning(message));
                    }
                }
            })
