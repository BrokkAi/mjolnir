...
With `TYPESAFE_API_KEY` set, or a key in `~/.secrets/typesafe_api_key`, Mjolnir automatically uses TypeSafe's Jev classifier to distinguish a question from ongoing work. It sends bounded recent prompt and assistant text, tool titles, and activity counts to TypeSafe after a minute of silence and after a completed reply. A confident user-input verdict marks the turn as waiting for you (`mj wait` returns `input_required`); the harness may still be running. A confident background-work verdict shows “expecting the agent to continue” until activity resumes. Missing keys, low confidence, and API failures keep the existing behavior. The daemon forwards its resolved key to container and SSH workers.
