...
When Jev changes a status, it is attributed as **Ready · Jev assessment** or **Expecting follow-up · Jev assessment**. Open **Jev decisions** from the session menu or TUI command palette, click an attributed status, or click an automatic-continuation notice to inspect what was checked, what Jev answered, and what mj actually did. Ordinary checks and retries do not add transcript messages.

The inspector lists pending, applied, unchanged, uncertain, stale, cancelled, and failed checks. Expand a check for its evidence scope and application outcome; technical details include scores, thresholds, timing, classifier questions, and the exact bounded JSON submitted. A positive assessment does not mean a worker accepted a continuation: these are recorded separately.

Exact inputs contain conversation text and live runtime facts. They are stored locally in `jev-decisions/decisions.*.jsonl` under the daemon data directory and each worker root, with four rotating 8 MiB segments per owner. HTTP authentication headers and configured TypeSafe keys are not recorded. The hosted proxy still does not log request content. Details can expire through rotation; the inspector reports **Details no longer available**, or an explicit retrieval error when a worker is unreachable. Older workers need to restart on updated code to supply these details. No permanent audit database or history backfill is created.
...
The session shows **Checking continuation** while Jev checks the conversation. A continuation appears as **Mj continued automatically · 1 of 3. Jev assessed that already-requested work remains.** Click the notice or open **Jev decisions** for the evidence and outcome. There are at most three automatic continuations between your messages. New input or interrupting the session cancels a pending check. Automatic turn review waits until the continuation chain settles.
