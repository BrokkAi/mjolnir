...
| Semantic help search | While you type in the help filter, 400 ms after the last key: the filter text and the text of every help row. | The same proxy (`/v1/help-search`), or `api.typesafe.ai` with your key. | No setting turns it off. It runs only while the help filter has text. |
