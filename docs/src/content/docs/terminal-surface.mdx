...
At startup, Mjolnir opens the session whose agent spoke most recently and puts the cursor in Prompt. Sessions shows every workspace and includes stopped sessions; opening a row switches to that conversation without changing its workspace ownership.
