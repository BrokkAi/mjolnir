...
Discrete review runs Bifrost through `npx -y @brokkai/bifrost@VERSION`, pinned
by default to the known-good version this mj release ships. A different version
selected on the Reviewer tab of `/mjconfig` replaces it for both CLI analysis
and MCP servers. On Linux, macOS, and Windows, Mjolnir uses `npx` from `PATH`
when available and otherwise installs an embedded Node.js 24 runtime
automatically. Android users already need Node.js/npm for the built-in ACP
bridges; the npm installation route provides that prerequisite by
construction.
