...
**prefix+s Settings → Machines and Runtimes**. No setup command is required. A target
that is checking or unavailable cannot advance; **prefix+shift+r** in the picker rechecks
all targets. Dashboard **prefix+shift+r** refreshes host capacity. The optional
