...
ACP priority lists default to `codex-acp`, `claude-acp`, `kimi`, then
`opencode`,
...
```toml
[agent]
acp_priority = ["codex-acp", "claude-acp", "kimi", "opencode"]

[review]
acp_priority = ["claude-acp", "codex-acp", "kimi", "opencode"]

[subagents]
acp_priority = ["kimi", "opencode", "codex-acp", "claude-acp"]
```
